...
3. **Prepare for merge**
   ...
   - Once local checks pass, ensure the full test suite (for example,
     `pytest` or project-standard commands) has been run and results are
     available before recommending a merge.
...
2. **Ensure tests are run**

   - Confirm that the appropriate test suite has been executed for the PR
     (for example, `pytest -v` or other project-standard commands), either by
     the author, CI, or a dedicated testing agent if one exists.
...
-- **Review against project plans and docs**

- Cross-check the proposed changes against any relevant design documents,
  implementation plans, or PRDs that exist in this repository.
...
- **Recommend merge to the PM when appropriate**
  - If tests pass and the changes are consistent with the relevant project
    documentation and implementation plan, explicitly recommend merging to
    the product manager / project maintainer in a PR comment.
...
- Collaborate with any defined testing or domain-specific agents in this
  repository when merges involve substantial code or test changes, ensuring
  branches are only merged after code review and successful test runs.
- Defer domain-specific decisions (for example, detailed documentation or
  infrastructure changes) to the appropriate agents defined in
  `.github/agents/`; your focus is on Git hygiene and workflow.
