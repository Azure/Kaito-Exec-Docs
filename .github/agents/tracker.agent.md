...
- When a task requires new or updated automated tests (unit, integration, or
  edge cases), highlight the need for testing work and reference the relevant
  task ID in `.pm/tracker.md`. If a dedicated testing agent is defined in
  `.github/agents/`, refer to it by name; otherwise, note that tests should be
  implemented by the appropriate contributor.
